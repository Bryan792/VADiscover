...
app.use(
  function responseTime(ctx, next){
    ...
    return next().then(function () {
      ...
      console.log('X-Response-Time', delta + 'ms');
    });
  }
);
...
router.get('/', (ctx, next) => {
    ctx.status = 200;
    ctx.body = 'Yello world from worker ' + (cluster.worker ? cluster.worker.id : '') + '!';
})

router.get('/mal', (ctx, next) => {
    ctx.status = 200;
    ctx.body = 'Yello world from worker ' + (cluster.worker ? cluster.worker.id : '') + '!';
})
